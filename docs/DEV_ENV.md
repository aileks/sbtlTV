...
- `SBTLTV_LOG_FILE=/tmp/sbtltv-app.log` write combined main+renderer logs.
...
## Linux playback (GStreamer appsink)
...
## YouTube resolving (all platforms)
...
## mpv (Windows/macOS)
- `SBTLTV_HWDEC=auto-safe|yes|no` mpv hwdec mode.
- `SBTLTV_MPV_VO=gpu|...` mpv video output override.
- `SBTLTV_MPV_GPU_CONTEXT=...` mpv GPU context override.
